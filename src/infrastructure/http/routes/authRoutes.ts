...
authRouter.get("/csrf", async (req: Request, res: Response) => {
  try {
    ...
    logger.error({
      type: "AUTH_CSRF_ERROR",
      message: "Erro ao gerar token CSRF",
      error: err,
    });
    res.status(500).json({
      error: { code: "INTERNAL_ERROR", message: "Erro ao gerar token CSRF" },
    });
  }
});
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.cookie("accessToken", tokens.accessToken, {
        httpOnly: true,
        secure: useSecureCookie, // HTTPS only em produção e não-localhost
        sameSite: "lax",
...
authRouter.post("/login", authLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // #region agent log
    void fetch(
      "http://127.0.0.1:7242/ingest/480d274d-bf63-41e3-b593-f2456c48c70b",
      {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          data: {
            useSecureCookie,
            cookieDomain,
            accessLen: tokens.accessToken?.length ?? 0,
            refreshLen: tokens.refreshToken?.length ?? 0,
          },
          timestamp: Date.now(),
        }),
      }
...
authRouter.post("/logout", async (req: Request, res: Response) => {
  try {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      logger.info({
        type: "AUTH_LOGOUT",
        message: "Token invalidated on logout",
    ...
    void fetch(
      "http://127.0.0.1:7242/ingest/480d274d-bf63-41e3-b593-f2456c48c70b",
      {
        ...
        body: JSON.stringify({
          ...
          data: {
            issuedAccess: Boolean(tokens?.accessToken),
            issuedRefresh: Boolean(tokens?.refreshToken),
...
authRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error({
        type: "AUTH_MFA_REQUEST_ERROR",
        message: "Erro ao solicitar MFA",
        error: err,
      });
      res
        .status(500)
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (
        ...
        return res.status(401).json({
          error: { code: "INVALID_OTP", message: "OTP inválido ou expirado" },
        });
      }
      logger.error({
        type: "AUTH_MFA_VERIFY_ERROR",
